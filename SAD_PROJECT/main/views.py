...
def add_contact_request(request):  # send a form
    if request.method == "POST":
        ...
        for id in checked_id:
            ...
            print(
                "click http://127.0.0.1:8000/accept_contact/"
                + str(request.user.id)
                + "/"
                + str(id)
            )
...
def group_member_request(request, group_id):
    ...
    acc = Account.get_account_by_user(request.user.id)
    context = {"contacts": Contact.get_contacts(acc), "g_id": group_id}
    return HttpResponse(template.render(context, request))
...
def done_group_member_request(request, group_id):
    ...
    for id in checked_id:
        ...
        print("click http://127.0.0.1:8000/accept_gp/"
              + str(group_id)
              + "/"
              + str(acc.user.id)
             )
        ...
        result = send_mail(
            "Inviting Group",
            "click http://127.0.0.1:8000/accept_gp/"
            + str(group_id)
            + "/"
            + str(acc.user.id),
            settings.EMAIL_HOST_USER,
...
def all_expenses(request):
    ...
    acc = Account.get_account_by_user(request.user.id)
    context = {
        "payed_payer": Expense.get_payed_payer_expenses(acc),
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return HttpResponseRedirect(
                reverse("main:add_share_member", args=(group_id, share.pk))
            )
    else:
        form = ShareForm()
        form.edit(my_choices=gp.get_members(gp, True))
    return render(request, "main/add_share.html", {"form": form, "group_id": group_id})
...
def add_share_member(request, group_id, share_id):
    ...
    if request.method == "POST":
        ...
        Share.add_shares(
            share_id, Account.get_account_by_user(acc_id), percent, amount_of_money
        )
    gp = Group.get_group(group_id)
    return render(
        request,
        "main/share_member.html",
        {
